...
This file is a template for IODA NetLogo-based models.

## HOW IT WORKS

In order to build a IODA NetLogo model, you have to:  

  1. include the `IODA_3_0.nls` file   
  2. use the `ioda` extension  
  3. define your interactions (e.g. in a text file, such as "interactions.txt")  
  4. define the interaction matrix and the update matrix (e.g. in a text file, such as "matrix.txt")  
  5. write the concrete primitives (reporters or commands) required so as to allow you breeds or patches to perform or undergo the interactions  
  6. and, of course, initialize the simulation.

## HOW TO USE IT

You can copy the whole folder and rename it to your project name, then modify the text files and the netlogo model according to the simulation you want. Please read the documentation (especially the [Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html)) provided with the IODA NetLogo extension for a detailed introduction to interaction-based simulation design within the NetLogo platform.

## THINGS TO NOTICE

During the setup of your own model, we recommand that you keep an output area and the **`show primitives to write`** button. This button runs the `setup` command and prints the result of the `ioda:primitives-to-write` reporter. Each time you modify your interactions or your matrices, please ensure that all concrete primitives are already written (in that case, nothing appears when you click the button). Otherwise, copy the result of the output to your "Code" tab and fill reporters and commands according to your needs.

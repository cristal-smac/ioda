...
This model is a very simple demonstration of basic requirements of the IODA NetLogo extension. In this model, particles move around and destroy one the other. It is the implementation of the first section of the  [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

## HOW IT WORKS

Use the **`setup`** button to initialize the simulation, then the **`go`** button to run. The simulation stops when the initial number of particles has been divided by 8.

## IODA NETLOGO FEATURES

This model demonstrates the basic operations to build a simulation with the IODA NetLogo extension. Especially:   

  * the `setup` procedure loads the interactions, the matrices, and ends with `ioda:setup`
  * the `go` procedure calls the `ioda:go` procedure to delegate the simulation to the IODA engine  
  * turtles can perceive their neighbors, thus a `turtles::filter-neighbors` procedure must be written  
  * turtles can perform several abstract action primitives (such as _wiggle_ and _die_) which are in the "actions" part of the interactions they can perform and undergo: therefore, the corresponding concrete action primitives (respectively `turtles::wiggle` and `turtles::die`) must be written

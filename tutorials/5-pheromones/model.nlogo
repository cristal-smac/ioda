...
matrix-file
matrix-file
"matrix.txt" "matrix-PRORATA.txt" "matrix-BEST.txt"
0

@#$#@#$#@
## WHAT IS IT?

This model explains how to use **Target Selection Policies** in IODA NetLogo, so as to specify how a source agent can use preferences when choosing a target for an interaction, based on target features. In that case, ants follow a pheromone gradient by interacting with patches which contain those pheromones.  
This is the implementation of the Section 5 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html). 

## HOW IT WORKS

Each patch contains an amount of pheromone. The highest level is found near the top left of the environment, and for the sake of clarity in the example, ants reaching this point simply die. Other behaviors are the following: when an ant does not feel any pheromone gradient, it moves randomly; otherwise, it moves to one of the neighboring patches with a higher level of pheromone.  
Naturally, at the end all ants move to the patch with the highest pheromone level and die. However, their paths are not very straight, since several neighboring patches can have a pheromone level higher than the level on the current patch, and thus be chosen as a target for the _`FollowGradient`_ interaction.   
IODA NetLogo provides Target Selection Policies so as to allow to specify preferences in the target selection. For instance, a convenient way to choose a patch as a target for the _`FollowGradient`_ interaction, consists in taking into account the amount a pheromone on the patch. This can be done either in a deterministic way (by choosing the patch with the highest amount) or in a probabilistic way (by choosing the patch with a probability proportional to the amount): i.e. respectively by using the **`BEST:pheromone`** or the **`PRORATA:pheromone`** target selection policy in the interaction matrix.

## HOW TO USE IT

First, select `"matrix.txt"` in the chooser, click on **`setup`**, then on **`go`**. Ants begin to move towards the patch with the highest pheromone level, but not in straight lines.  
Then, select `"matrix-PRORATA.txt"`, **`setup`**, **`go`**. Paths are now a bit more direct.  
Finally, try `"matrix-BEST.txt"`, **`setup`**, **`go`**. Now ants go directly from one patch to the neighbor with the highest pheromone level. 

## IODA NETLOGO FEATURES

The concept of Target Selection Policy (see Section 5 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html)).

## RELATED MODELS

Other Target Selection Policies are presented in the "explosion" model (see Section 8 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html)).

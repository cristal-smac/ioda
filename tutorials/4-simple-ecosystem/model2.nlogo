...
globals [hunted-sheep sheep-births season growth-rate season-name]

breed [wolves wolf]
breed [sheep a-sheep]

wolves-own  [last-dinner]
sheep-own   [stomach age female?]
patches-own [food]
...
to setup
  ...
  init-agents
  ioda:load-interactions "interactions2.txt"
  ioda:load-matrices "matrix2.txt" " \t,"
  ioda:setup
  reset-ticks
end

to go
  set season (round ticks / season-duration) mod 4
  set growth-rate item season [ 0.1 0.7 0.5 0.2]
  set season-name item season [ "winter" "spring" "summer" "fall" ]
  set hunted-sheep 0
  set sheep-births 0
  ioda:go
  tick
...
to recolor-patch
  set pcolor scale-color (item season [gray lime green orange]) food 0 (max-food * 1.2)
end
...
to init-sheep
  ...
  set color white
  set size 1.5
  set stomach random 2 * sheep-hunger
  set age adult-sheep
  set female? random 2 = 0
end

to init-wolf
  set last-dinner (- random wolves-hunger)
  setxy random-xcor random-ycor
  set color brown
  set size 2
end
...
to-report wolves::hungry?
  report ticks - last-dinner >= wolves-hunger
end
...
to-report sheep::alone?
  report not any? other sheep in-radius 1
end

to-report sheep::hungry?
  report stomach <= sheep-hunger
end
...
to sheep::reproduce
  ...
  hatch-sheep 1 [ ioda:init-agent set age 0 set heading random 360 fd 0.8 set color white set size 1
    set stomach 10 set female? (random 2 = 0) create-link-with myself [ tie ]]
end

to sheep::decrease-health
  ifelse female? [ set stomach stomach - 8]
    [ set stomach stomach - 4]
end
...
to patches::increase-size
  set food food + growth-rate
  recolor-patch
end

to-report sheep::female?
  report female?
end

to-report sheep::male?
  report not sheep::female?
end

to-report sheep::has-child?
  report any? link-neighbors
end

to-report sheep::few-children?
  report count my-links < 2
end

to-report sheep::grown?
  report sheep::adult?
end

to sheep::increase-size
  set age age + 1
  if (age >= adult-sheep)
    [ set size 1.5
...
to-report sheep::starving?
  report stomach <= 0
end

to sheep::nurse
  let t ioda:my-target
  let q 1 + round(stomach / 2)
  set stomach stomach - q
  ask t [set stomach stomach + q]
end

to sheep::consume-energy
  set stomach stomach - 1
end
...
"" ""
PENS
"newborn sheep" 1.0 1 -11221820 true "" "plot count sheep with [not sheep::grown?]"
"adult sheep" 1.0 0 -13345367 true "" "plot count sheep with [sheep::adult?]"
"adult females" 1.0 0 -13791810 true "" "plot count sheep with [sheep::adult? and female?]"
...
This model presents a simple ecosystem with three agent families: grass (patches), sheep (a NeLogo breed) and wolves (another breed). It is the implementation of the enhancement proposed in the section 4 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).
...
This model is an implementation of those extensions.
...
This model is close to many predator/prey simulations, such as the "Wolf Sheep Predation" model in the NetLogo library, though it is not aimed at demonstrating equilibria but rather IODA features. It is an extension of the file [`"model1.nlogo"`](file:model1.nlogo).

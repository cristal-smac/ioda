...
to-report shannon-entropy
  report (log nb-chips 2) - (sum [ifelse-value any? chips-here
        [(count chips-here) * (log (count chips-here) 2)]
        [0]] of patches) / nb-chips
end
...
to termites::drop-load
  set carrying? false
  set color white
  hatch-chips 1 [set color yellow ioda:init-agent]
end
...
"theoretical-max" 1.0 0 -2674135 true "" "plot log count patches 2"

@#$#@#$#@
## WHAT IS IT?

This model is a new version of the "Termites" model, in which termites move true wood chips instead of coloring patches. It is the implementation of Section 3 in the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

## HOW IT WORKS

In this model we have two families of agents (i.e. two NetLogo breeds): `termites` and `chips`. Termites can perform interactions on chips, such as _`PickUp`_. They also can perform reflexive interactions (they act alone), such as _`MoveRandomly`_. 

## THINGS TO NOTICE

The _`MoveRandomly`_ interaction is used both as a reflexive interactions (termites move randomly when they cannot perform any other interaction) and with chips as targets: in that case, termites can perform _`MoveRandomly`_ only when there is some chip nearby. This produces a 'move away from chips' behavior.

The plot measures the "Shannon Entropy" or "Information Entropy" related to the positions of chips. It is given by: H = log<sub>2</sub> Q - (&Sigma; Q<sub>i</sub> log<sub>2</sub> Q<sub>i</sub>)/Q  where Q is the number of chips and Q<sub>i</sub> the number of chips on patch _i_. When all chips are uniformly distributed on the patches, the theoretical value is log<sub>2</sub>N where N is the number of patches. It decreases while the termites move the chips, towards a value that depends on the sparseness of the piles.

## EXTENDING THE MODEL

* Change 0.3 to 0.8 in the interaction matrix: now you should get sparse piles instead of compact piles.
* Try to adapt the model so as to handle multiple materials (e.g wood chips, food, eggs, etc.).

## RELATED MODELS

The NetLogo "Termites" model and its adaptation (see Section 2 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html)).

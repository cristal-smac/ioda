...
to setup
  clear-all
  set cols [ yellow blue red green orange ]
...
to init-termite
  ...
  set color (one-of sublist cols 0 nb-colors) - 3
  set carrying? false
end

to init-chip
  setxy random-xcor random-ycor
  set color (one-of sublist cols 0 nb-colors)
end
...
This model is a modification of the model presented in tutorial `#3-termites with chips`, with colored wood chips and colored termites. It is aimed at demonstrating the perception primitives introduced since version 2.3.

## HOW IT WORKS

In this model wood chips have a specific color, an so do termites. Our goal is that termites of a given color handle only chips with the same color. A first solution would be to **modify** the interactions by adding a color-dependent condition. 
Or, we could define separate breeds for each color. 

The solution proposed here consists in changing the perception of agents: termites are designed to perceive only agents of the same color (actually, shade). This can be done through the perception of the termites. Combined with `ioda:filter-neighbor-`, `ioda:add-neighbors-`, `ioda:remove-neighbors-` and `ioda:retain-only-neighbors`, the `custom` primitive allows to use any reporter to tune the perception. This reporter has three arguments: the source agent, the target agent and a list of parameters.

Here for instance, termites use the "same-color?" reporter which reports whether a termite and a wood chip have similar colors or not:

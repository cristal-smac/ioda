...
extensions [ioda sound]

breed [mines mine]
breed [soldiers soldier]
breed [halos halo]

mines-own [activated? counter]
soldiers-own [strength shield?]
...
to setup
  ...
  ask patches with [pxcor != 0 or pycor != 0]
    [ sprout-soldiers 1 [ init-soldier ]]
  let risk min (list 4 mine-radius)
  create-mines 1 [ init-mine move-to patch 0 0 ]
  ioda:load-interactions "interactions.txt"
  ioda:load-matrices (word experiment ".txt") " \t"
  ioda:setup
  ioda:set-metric metric
  ask patches with [ioda:distance patch 0 0 <= risk]
    [set pcolor red + 2]
  ask patches with [(ioda:distance patch 0 0 > risk) and (ioda:distance patch 0 0 <= mine-radius)]
    [set pcolor green + 2]
  reset-ticks
end
...
to init-soldier
  ioda:init-agent
  set strength 1 + random-poisson 4
  set label strength
  set shield? (random 10 = 0)
  set color ifelse-value shield?
    [ yellow ]
    [ scale-color blue strength 0 10 ]
end
...
to mines::filter-neighbors
  ioda:filter-neighbors-in-radius mine-radius
end

to-report soldiers::no-shield?
  report not shield?
end

to-report soldiers::weakness
  report max [strength] of soldiers - strength
end

to-report mines::not-activated?
  report not activated?
end

to-report mines::countdown-finished?
  report counter = 0
end

to mines::count-down
  if (counter > 0) [ set counter counter - 1  set label counter ]
end

to-report mines::activated?
  report activated?
end
...
to-report mines::distance-strength-tradeoff
  let t sort-by [ [?1 ?2] -> distance ?1 < distance ?2] ioda:my-target
  let r []
  let s 50
  while [(not empty? t) and (s > [strength] of first t)]
    [ set s s - [strength] of first t
      set r fput first t r
      set t but-first t ]
  report r
end

to-report mines::random-number
  let n length ioda:my-target
  report n-of (random n) ioda:my-target
end
@#$#@#$#@
GRAPHICS-WINDOW
182
...
165
271
nb soldiers without shield
count soldiers with [(shade-of? pcolor red) and (not shield?)]\n
0
1
11
...
experiment
experiment
"RANDOM" "RANDOM-INT" "BEST" "PRORATA" "ALL" "ALL-BEST" "NUMBER10-" "NUMBER-10" "NUMBER45-" "FILTER1" "FILTER2"
0
...
This model is designed to demonstrate how **Target Selection Policies** work. It shows the explosion of a mine which can blast soldiers in the neighborhood. This is the implementation of the Section 8 of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

## HOW IT WORKS

The model is provided with several interaction matrices corresponding to each Target Selection Policy. For detailed explanations, please report to the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html) (Sections 5: "pheromones" and 8: "explosion"). When the mine explodes, it can blast one ore more soldiers among those who are not protected by a special shield (shown in yellow). Target Selection Policies allow to define how many soldiers, and according to what criterion, are likely to be the target of the "Blast" interaction.

## HOW TO USE IT

  * Select an experiment (i.e. an interaction/update matrix file).  
  * Click on **`setup`**. The green area represents the perception halo of the mine, while the red area corresponds to the limit distance of the _`Blast`_ interaction: i.e., only soldiers within the red area can be the target of _`Blast`_. Soldiers in yellow are protected by a shield and thus cannot be killed. The number near each soldier represents its strength: some policies use the strength to determine what soldiers will be killed.    * Click on **`go`**. Depending on the experiment (thus on the Target Selection Policy), one or more soldiers can be killed.  
  * You can retry the experiment with another matrix file. You can also use one of the 3 usual metrics which are pre-defined in IODA NetLogo (euclidean, Moore and Von Neumann).

## IODA NETLOGO FEATURES

This model gives a code example for each Target Selection Policy provided in the IODA NetLogo extension.  
It also introduces the use of **metrics**. By default (at the end of the `ioda:setup` procedure), the `ioda:metric` variable is set to `"Euclidean"`, thus the `ioda:distance` is computed according to the Euclidean metric (d(point1, point2) = sqrt((x1-x2)<sup>2</sup> + (y1-y2)<sup>2</sup>)). The IODA NetLogo engine allows to use either the Moore metric (d(p1,p2) = max(|x1-x2|, |y1-y2|)) or the Von Neumann metric (d(p1,p2) = |x1-x2| + |y1-y2|) instead. You just have to call `ioda:set-metric` with the name of the metric you need (`"Euclidean"`, `"Moore"` or `"Von Neumann"`).

## RELATED MODELS

See also the model provided in section 5 (["pheromones"](file:../5-pheromones/model.nlogo)) of the tutorial files, for simple and frequently used Target Selection Policies.  
See also the "Moore & Von Neumann Example" in the NetLogo library (Code Examples) for the difference between those metrics.
...
@#$#@#$#@
<experiments>
  <experiment name="frequencies" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="11"/>

...
to setup
  ...
  create-turtles 100
    [ setxy random-xcor random-ycor
      ...
      set color white
      set carrying? false ]
  ask n-of 2000 patches [set pcolor yellow]
...
This model is a IODA NetLogo version of the classical NetLogo "Termites" model. It is the implementation of Section 2 in the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

## HOW IT WORKS

The termites walk around, collect wood chips (represented by yellow patches) and put down chips when they find another chip.

## HOW TO USE IT

Just click on the **`setup`** button, then on the **`go`** button. 

## THINGS TO NOTICE

There are several important differences between this model and the NetLogo Termites model:  

  * behaviors are not tangled with the code, but defined in an abstract way in the "interactions.txt" file  
  * the association between behaviors and agents is ensured by the "matrix.txt" file
  * agents need concrete primitives (i.e. commands and reporters) to be able to perform or undergo the interactions, thus the NetLogo code contains several procedures of the form `turtle::XXX`.

## EXTENDING THE MODEL

See section 3 in the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

## RELATED MODELS

  * The "Termites" model in the NetLogo Library  
  * The "termites-with-chips" model in the next section of the [IODA NetLogo Tutorial](file:../../doc/IODA-NetLogo-Tutorial.html).

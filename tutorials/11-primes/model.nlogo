...
turtles-own [ value min-x min-y max-x max-y]
nbs-own [ destination ]
...
to go
  ioda:go
  tick
  if ((any? generators with [value >= limit]) and (not any? nbs))
    [stop]
end


to init-world
  set-default-shape generators "arrow"
  set-default-shape nbs "dot"
  set-default-shape primes "arrow"
  ask patch 0 0 [ sprout-generators 1 [ set color white set heading 90 set value 2 ]]
end

to-report generators::at-limit?
  report value >= limit
end

to-report nbs::arrived?
  report distance destination <= 0.1
end

to-report primes::is-divisor?
  let n [value] of ioda:my-target
  report (n mod value = 0)
end


to primes::filter-neighbors
  ioda:filter-neighbors-on-patches (patch-set patch-here)
end

to generators::send-number
  let v value
  hatch-nbs 1 [ ioda:init-agent set destination patch-ahead 2 set color yellow set value v set label v ]
end

to generators::increase-value
  set value value + 1
end
...
to nbs::create-prime
  hatch-primes 1 [ ioda:init-agent set color red move-to patch-here compute-heading ]
end

to primes::transmit-heading
  let h heading
  let x1 min-x let x2 max-x let y1 min-y let y2 max-y
  ask ioda:my-target [ set heading h set destination patch-ahead 2
    set min-x x1 set max-x x2 set min-y y1 set max-y y2 ]
end

to-report nbs::prime-here?
  report any? primes-on patch-here
end

to-report generators::check-period?
  report ticks mod generator-period = 0
end

to compute-heading
  ifelse (any? other primes)
    [ if ((xcor != 0) and ((xcor > max-x) or (xcor < min-x)))
       or ((ycor > max-y) or (ycor < min-y))
         [ left 90 ]
         if (xcor != 0 and xcor > max-x) [ set max-x xcor ]
         if (xcor != 0 and xcor < min-x) [ set min-x xcor ]
         if (ycor > max-y) [ set max-y ycor ]
         if (ycor < min-y) [ set min-y ycor ]
    ]
    [ left 90 ]
end
@#$#@#$#@
...
"" ""
PENS
"numbers left" 1.0 0 -16777216 true "" "plot count nbs"
"primes found" 1.0 0 -2674135 true "" "plot count primes"

@#$#@#$#@
## WHAT IS IT?

This model is a distributed and spatialized computation of prime numbers, based on Eratosthene's algorithm.

## HOW IT WORKS

The generator in the center of the environment creates numbers from 2 to the specified limit. Each number moves forward; if it reaches a position without any prime number, it becomes prime itself. When numbers encounter primes, they can be killed (if the prime is one of their divisors), or redirected. Thus, prime numbers progressively settle in the space within a spiral shape.

## HOW TO USE IT

Click on **`setup`**, then **`go`**. You can also raise the generator period and lower the simulation speed so as to follow moving numbers more clearly.
